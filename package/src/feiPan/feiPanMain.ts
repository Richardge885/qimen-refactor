...
export function paiFeiPan(
    ...
    let result: PanJuInformation = initializeJiuGong(
        year,
        month,
        date,
        hour,
    ...
    result.panJuResult = putInLiuQin(
        result.allTimeInformation.shizhu.charAt(0),
        result.panJuResult,
    ); // 给盘踞信息添加六亲信息
    ...
    if (
        ...
        result.huanJu = inputSymboleInfo(
            result.huanJu,
            result.allTimeInformation.shizhu.charAt(1),
        ); // 给盘踞信息添加辅格信息
    }
...
function initializeJiuGong(
    ...
    return {
        ...
        zhiFu: zhiFu,
        zhiShi: zhiShi,
        qiJuMethod: baoshuMethod,
        qiJuNumber: baoshu,
        choosenJuShu: ziXuanJu,
        huanJu: {
            kanGong: {
                baGua: '坎',
                gongWangShuai: '',
