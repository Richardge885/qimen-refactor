 ...
 *
 */
export function getTimeInfo(time: PaipanTime, baoshuMethod: string, baoshu: number) {
    let solarToLunar;
    if (baoshuMethod == '制筹' || baoshuMethod == '时辰') {
        if (baoshuMethod == '制筹') {
            // 根据六十甲子制筹选时
            const solar = Solar.fromYmdHms(time.year, time.month, time.day, time.hour, 0, 0);
            ...
            const solar = Solar.fromYmdHms(time.year, time.month, time.day, hour!, 0, 0);
            solarToLunar = solar.getLunar();
        }
    } else {
