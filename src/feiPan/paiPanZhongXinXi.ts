import { rearrangeArray } from "../common-methods/commonPaipanFunctions";

export function paiTianPanGan(
    zhiFu: string,
    dun: string,
    xing: string[],
    xunShou: string,
...
export function paiTianPanShen(zhiFu: string, dun: string, xing: string[]): string[] {
    const yinDunShen = [
        '值符',
        '九天',
        '九地',
...
export function getZhiShiPosition(anGanZhi: string[], shizhu: string) {
    let position = 0;
    switch (shizhu) {
        default:
            break;
...
export function paiBaMen(zhiShi: string, zhiShiPosition: number) {
    let baMenList;
    let count = 0;
    switch (zhiShi) {
        case '休门':
...
export function paiXing(zhiFu: string, diPanGanList: string[], shizhu: string) {
    let xingList;
    let shigan;
    if (zhiFu == '天蓬') {
        xingList = [
